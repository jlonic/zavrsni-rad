...
const getTopAlbums = async (_, res) => {
    try {
        ...
        exec("/usr/bin/python3 ./python-scripts/getTop200Albums.py", (error, data) => { //for running locally
            if (error) {
...
const getHot100 = async (_, res) => {
    try {
        ...
        exec("/usr/bin/python3 ./python-scripts/getHot100.py", (error, data) => { //for running locally
            if (error) {
...
const getGlobal200 = async (_, res) => {
    try {
        ...
        exec("/usr/bin/python3 ./python-scripts/getGlobal200.py", (error, data) => { //for running locally
            if (error) {
                return res.status(500).send({ message: "Error fetching top songs" });
            }
            res.json(JSON.parse(data));
...
const getTop100Artists = async (_, res) => {
    try {
        ...
        exec("/usr/bin/python3 ./python-scripts/getTop100Artists.py", (error, data) => { //for running locally
            if (error) {
