...
const Moderator = () => {
    ...
    const getAllReports = async () => {
        const token = localStorage.getItem('token');
        try {
            const response = await fetch('https://zavrsni-rad-backend-3u9x.onrender.com/reports/getReportsAndReviews', {
                method: 'GET',
                headers: {
    ...
    const deleteReport = async (report_id) => {
        const token = localStorage.getItem('token');
        try {
            const response = await fetch('https://zavrsni-rad-backend-3u9x.onrender.com/reports/deleteReport', {
                method: 'DELETE',
                headers: {
                    'Content-Type': 'application/json',
                    'Authorization': `Bearer: ${token}`,
