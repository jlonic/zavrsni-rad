...
const Album = () => {
    ...
    const fetchAlbumReviews = useCallback(async () => {
        try {
            const response = await fetch(`https://zavrsni-rad-backend-3u9x.onrender.com/reviews/album/${album_id}`, {
                method: 'GET',
                headers: {
    ...
    useEffect(() => {
        ...
        const fetchAlbum = async () => {
            try {
                ...
                if (response.ok) {
                    ...
                    const favoriteStatusResponse = await fetch(`https://zavrsni-rad-backend-3u9x.onrender.com/favorites/check-album-status/${album_id}`, {
                        method: 'GET',
                        headers: {
                            'Content-Type': 'application/json',
        ...
        const fetchTracks = async () => {
            try {
                const response = await fetch(`https://zavrsni-rad-backend-3u9x.onrender.com/tracks/album/${album_id}`, {
                    method: 'GET',
                    headers: {
                        'Content-Type': 'application/json',
                    },
        ...
        const fetchAlbumRating = async () => {
            try {
                const response = await fetch(`https://zavrsni-rad-backend-3u9x.onrender.com/albums/rating/${album_id}`, {
                    method: 'GET',
                    headers: {
                        'Content-Type': 'application/json',
                    },
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            const response = await fetch('https://zavrsni-rad-backend-3u9x.onrender.com/reviews/reviewAlbum', {
                method: 'POST',
                headers: {
                    'Content-Type': 'application/json',
                    'Authorization': `Bearer: ${token}`
    ...
    const addToFavorites = async () => {
        const token = localStorage.getItem('token');
        try {
            const response = await fetch(`https://zavrsni-rad-backend-3u9x.onrender.com/favorites/add-favorite-album/`, {
                method: 'POST',
                headers: {
                    'Content-Type': 'application/json',
                    'Authorization': `Bearer: ${token}`
    ...
    const removeFromFavorites = async () => {
        const token = localStorage.getItem('token');
        try {
            const response = await fetch(`https://zavrsni-rad-backend-3u9x.onrender.com/favorites/remove-favorite-album/`, {
                method: 'DELETE',
                headers: {
                    'Content-Type': 'application/json',
                    'Authorization': `Bearer: ${token}`
                                ...
                                <div className='flex-col justify-start mt-4'>
                                    {tracks.map((track) => (
                                        <div key={track.track_id} className="mt-4">
                                            <h4>
                                                {track.track_number}.{' '}
                                                <a href={`https://zavrsni-rad-two.vercel.app/tracks/${track.track_id}`} className="hover:underline">
                                                    {track.track_title}
                                                </a>{' '}
