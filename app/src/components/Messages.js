...
const Messages = () => {
    ...
    const getMessages = useCallback(async () => {
        ...
        try {
            const response = await fetch(`https://zavrsni-rad-backend-gdih.onrender.com/messages/getMessagesWithUser/${receiver_id}`, {
                method: 'GET',
                headers: {
            ...
            if (response.ok) {
                const getuser = await fetch(`https://zavrsni-rad-backend-gdih.onrender.com/users/get/${receiver_id}`, {
                    method: 'GET',
                    headers: {
                        'Content-Type': 'application/json',
                        'Authorization': `Bearer ${token}`,
    ...
    const sendMessage = async (e) => {
        ...
        try {
            const response = await fetch(`https://zavrsni-rad-backend-gdih.onrender.com/messages/sendMessage`, {
                method: 'POST',
                headers: {
                    'Content-Type': 'application/json',
                    'Authorization': `Bearer ${token}`
    ...
    return (
        <div className='bg-gray-700 min-h-screen'>
            ...
            <div className='text-gray-200 p-4'>
                <h1>
                    <img src={`https://zavrsni-rad-backend-gdih.onrender.com${profilePicture}`} alt='' className="w-12 h-12 rounded-full"></img>{username}
                </h1>
