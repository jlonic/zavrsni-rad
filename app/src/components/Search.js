...
const Search = () => {
    ...
    const handleChange = async (e) => {
        ...
        if (query.length > 1) {
            try {
                const response = await fetch(`https://zavrsni-rad-backend-3u9x.onrender.com/search/${category}/${query}`, {
                    method: 'GET',
                    headers: {
                                ...
                                result.username && (
                                    <div key={index} className='px-4 py-2 border-b border-gray-200'>
                                        <a href={`https://zavrsni-rad-two.vercel.app/users/${result.username}`} className='flex items-center space-x-2'>
                                            <img src={`https://zavrsni-rad-backend-3u9x.onrender.com${result.profile_picture}`} alt='' className="w-10 h-10 mr-2 rounded-full" />
